...
TROOPS_TO_DO_REGIME_CHANGE = 6
TROOPS_UPKEEP_REGIME_CHANGE = 5
MAX_PLOTS = 12
MAX_DISRUPT = 2
MIN_DISRUPT = 1
WOI_DIVIDE = 5
...
class Country:
  ...
  def jihad_presence_Q(self): return self.cadre_Q() or self.totalCells() > 0
  def plot_Q(self): return self.plots > 0
...
#        app.handleMuslimWoI(modRoll, target)
        res = app.war_of_ideas(target)
        if res[0] == False :
          if res[2] == 'ineligble_woi' :
            print("   War of Ideas must target muslim or non-muslim country (7.2)")
          elif res[2] == 'alignment' :
            print("   %s alignment must be NEUTRAL or ALLY")
          elif res[2] == 'regime_change_troop_restriction' :
            print("   Only %d troops to %d cells (7.2.1.3)")
        elif res[0] == True :
          if res[2] == 'success' :
            print("   War of Ideas success")
          if res[2] == 'success_prestige' :
            print("   War of Ideas success: increase prestige (+1)")
          elif res[2] == 'failed_aid' :
...
class TroopTrack():
  def __init__(self, num) :
    self.name = 'troop_track'
    self.__troops = num
    self.__troops_max = num

  def troops(self) : return self.get_troops()
...
class Board():
  def __init__(self, scenario, world_config, theapp) :
    ...
    self.__available_plots = MAX_PLOTS
    self.deck = {}
    self.gwot_relations = { }
    self.app = theapp
  ...
  def activate_sleepers(self, country, num = CELLS_MAX) :
  ...
  def remove_active_cells(self, country, num = 1) :
    ...
    if self.world[country].active_cells <= 0 : self.add_cadre(country)
    return a

  def place_cells(self, dst, num, src, active = False):
    def f(dst, num) :
      if dst != 'funding_track' and src[0] == 'funding_track' :
        n = min(self.funding_track.available_cells(), num)
        if active :
          self.world[dst].active_cells = self.funding_track.remove_cells(n)
        elif not active :
          self.world[dst].sleeper_cells = self.funding_track.remove_cells(n)
        self.remove_cadre(dst)
        return (dst, num)
      elif dst == 'funding_track' and src[0] != 'funding_track' :
        n = min(getattr(self.world[src[0]], src[1]), num)
        self.funding_track.add_cells(n)
        setattr(self.world[src[0]], src[1], n)
        return (dst, num)

    if dst == 'funding_track' : return f(dst, num)
    self.test_country(dst, f, num)
  ...
  def available_plots(self) : return self.__available_plots

  def add_plots(self, country, num = 1) :
    def f(country, num) :
      self.world[country].plots += num
      self.__available_plots -= num
      return self.world[country].plots

    if num < self.__available_plots :
      return self.test_country(country, f, num)
    raise Exception("No plots available to add to %s!" % country)

  def remove_plots(self, country, num = 1) :
    def f(country, num) :
      self.world[country].plots = max(self.world[country].plots - num, 0)
      self.__available_plots += num
      return self.world[country].plots

    if num + self.__available_plots > MAX_PLOTS :
  ...
  def set_country_event(self, country, events) :
    def f(country, events) :
      if type(events).__name__ == 'list' :
        ...
        for event in events :
          if 'besieged' in event : self.world[country].besieged = 1
          if 'regime_change' in event : self.world[country].regime_change = 1
          if 'aid' in event : self.world[country].aid += 1

      else:
        self.world[country].markers.append(events)
        if 'besieged' == events : self.world[country].besieged = 1
        if 'regime_change' == events : self.world[country].regime_change = 1
        if 'aid' == events : self.world[country].aid += 1
    ...
    def f(country, events) :
      if type(events).__name__ == 'list' :
        for x in events :
          ...
          if 'besieged' in events : self.world[country].besieged = 0
          if 'regime_change' in events : self.world[country].regime_change = 0
          if 'aid' in events : self.world[country].aid = max(self.world[country].aid - 1, 0)

      else:
        self.world[country].markers.remove(events)
        if 'besieged' == events : self.world[country].besieged = 0
        if 'regime_change' == events : self.world[country].regime_change = 0
        if 'aid' == events : self.world[country].aid = max(self.world[country].aid - 1, 0)
  ...
  def get_neutral_countries(self) :
    return [ c for n,c in self.world.items() if c.neutral_Q() ]

  def get_non_muslim_countries(self) :
    return [ c for n,c in self.world.items() if c.soft_Q() or c.hard_Q() ]

  def get_countries_with_plots(self) :
    l = []
    for n,c in self.world.items() :
      if c.plot_Q() : l.append(c)
    return l
  ...
  def get_countries_with_cells(self) :
    return [c for n,c in self.world.items() if c.totalCells() > 0]

  def get_adjacent_countries(self, c_name) :
    return [ self.country(c.name) for c in self.country(c_name).adjacent_countries ]
...
#        self.outputToHistory(self.board.country_summary(country), True)
  ...
  def executeJihad(self, country, rollList):
    ...
    if isMajorJihad: # all cells go active
      ...
      for i in range(len(rollList) - self.map[country].num_active_cells()):
    ...
    if self.ideology <= 4:
      for i in range(failures):
        if self.map[country].num_active_cells() > 0:
          self.board.remove_active_cell(country)
        else:
          self.map[country].sleeper_cells -= 1
  ...
  def placePlots(self, country, rollPosition, plotRolls, isMartydomOperation = False, isDanishCartoons = False, isKSM = False):
    if (self.map[country].totalCells(True)) > 0:
      if isMartydomOperation:
        ...
        for i in range(plotsToPlace - self.map[country].num_active_cells()):
          self.outputToHistory("Cell goes Active", False)
          self.map[country].sleeper_cells -= 1
          self.map[country].active_cells += 1
...
#             self.map[location].sleeper_cells -= 1
  ...
  def war_of_ideas(self, c_name) :
    country = self.board.country(c_name)
    if c_name == "United States" or country.iran_Q() or c_name == 'Israel':
      return (False, c_name, 'ineligible_woi')
    elif (country.shia_mix_Q() or country.suni_Q()) and country.alignment <= Alignment.ADVERSARY :
      return (False, c_name, 'alignment')
    elif country.ally_Q() and country.regime_change_Q() and country.troops() - country.totalCells() < TROOPS_UPKEEP_REGIME_CHANGE :
      return (False, c_name, 'regime_change_troop_restriction')

    if country.shia_mix_Q() or country.suni_Q() :
      if country.neutral_Q() :
        m_roll = sum([
          self.board.prestige_track.get_prestige_modifier()[1]
          , self.board.gwot()['penalty']
          , country.aid
          , WOI_ADJ_GOOD if any([ c.good_Q() for c in self.board.get_adjacent_countries(c_name)]) else 0
          , random_roll()
        ])
        if m_roll >= WOI_DIVIDE :
        ...
        m_roll = sum([
          ...
          , country.aid
          , WOI_ADJ_GOOD if any([ c.good_Q() for c in self.board.get_adjacent_countries(c_name)]) else 0
          , WOI_TO_GOOD if country.fair_Q() else 0
          , random_roll()
        ])
        if m_roll >= WOI_DIVIDE :
          if country.poor_Q() :
    ...
    elif country.non_muslim_Q() :
      if random_roll() <= POSTURE_DIVIDE : self.board.set_posture(c_name, Posture.SOFT)
      elif random_roll() > POSTURE_DIVIDE : self.board.set_posture(c_name, Posture.HARD)
      if self.board.country(c_name).posture == self.board.country("United States").posture :
        self.board.prestige_track.inc_prestige()
        return (True, c_name, 'success_prestige')
      return (True, c_name, 'success')
  ...
  def disrupt(self, c_name, d_actives = 0, d_sleepers = 0) :
    ...
    country = self.board.country(c_name)
    if d_actives > 0 and country.active_cells < 1:
      return (False, c_name, 'no_active_cells')
    elif d_sleepers > 0 and country.sleeper_cells < 1:
      return (False, c_name, 'no_sleeper_cells')
    elif not country.non_muslim_Q() and country.troops() == 0 and not country.ally_Q() :
      return (False, c_name, 'ineligible_disrupt')
    elif not country.jihad_presence_Q() :
      return (False, c_name, 'no_jihad_presence')
    elif d_actives == 0 and d_sleepers == 0 and not country.cadre_Q() :
      return (False, c_name, 'no_cadre')
    ...
    if not self.board.event_in_play_Q("Al-Anbar") and d_actives == d_sleepers and d_actives == 0 :
      ...
      d_limit = max([
        MAX_DISRUPT if country.troops() >= 2 else MIN_DISRUPT
        , MAX_DISRUPT if country.hard_Q() else MIN_DISRUPT
      ])

      if self.board.event_in_play_Q("Al-Anbar") and (country.name == "Iraq" or country.name == "Syria") :
        d_limit = MIN_DISRUPT

      actives = country.active_cells
      sleepers = country.sleeper_cells
      if d_actives > actives : return (False, c_name, 'not_enough_actives')
  ...
  def alert(self, country) :
    if not self.board.country(country).plot_Q() :
      return (False, country, 'no_plots')

    return (True, country, self.board.remove_plots(country))

  def reassessment(self) :
    if self.board.country("United States").hard_Q():
      self.board.set_posture("United States", Posture.SOFT)
    else:
      self.board.set_posture("United States", Posture.HARD)
    return [True]
  ...
  def deploy(self, dst, num, src) :
    ...
    if src != 'troop_track' and self.board.country(src).regime_change_Q() :
      ...
      if (c.troops_stationed - num) < (c.totalCells(True) + TROOPS_UPKEEP_REGIME_CHANGE) :
  ...
  def regime_change(self, dst, num, src) :
    if not self.board.country('United States').hard_Q() :
      return (False, dst, num, src, 'us_posture_soft')
    elif self.board.country(dst).regime_change_Q() :
      return (False, dst, num, src, 'already_regime_change')
    elif src != 'troop_track' and num < TROOPS_TO_DO_REGIME_CHANGE and self.board.country(src).troops_stationed >= num :
      return (False, dst, num, src, 'min_troops')
    elif src == 'troop_track' and num < TROOPS_TO_DO_REGIME_CHANGE and self.board.troop_track.get_troops() >= num :
      return (False, dst, num, src, 'min_troops')
    elif not self.board.country(dst).islamist_rule_Q() :
      return (False, dst, num, src, 'not_islamist_rule')
    elif src == 'troop_track' and self.board.troop_track.get_troops() < num :
      return (False, src, 'not_enough')
    elif src != 'troop_track' and self.board.country(src).troops_stationed < num :
      return (False, dst, num, src, 'not_enough')
    elif src != 'troop_track' and self.board.country(src).regime_change_Q() :
      c = self.board.country(src)
      if (c.troops_stationed - num) < (c.totalCells(True) + TROOPS_UPKEEP_REGIME_CHANGE) :
        return (False, src, 'regime_change_troop_restriction')
    ...
    x, t = self.board.place_troops(dst, num, src)
    if t != num : raise Exception("The request number of units to deploy could not be satisfied")
    if random_roll() <= GOVERNANCE_DIVIDE : self.board.set_governance(dst, Governance.POOR)
...
class UICmd(cmd.Cmd) :
  ...
  def do_status(self, args) :
    if len(args) == 0 :
      ...
      print(self.game.board.country_summary(args.lower().title()))

  def complete_status(self, text, line, begidx, endidx) :
    return [ c for c in list(self.game.board.world.keys()) if c.lower().startswith(text.lower()) ]

  def help_status(self):
    print("    Display status of game or single country")
    print("    Usage: status OR status <country>\n")
    print("    Example: status Saudi Arabia\n")

  def parse_target_country(self, line, params) :
    m = re.match('([a-zA-Z/_]+( [a-zA-Z]+)?)$', line.strip())
    if m == None : return False

    params['country'] = m.group(1).lower().title()
    return True

  def parse_disrupt(self, line, params) :
    m = re.match('([a-zA-Z/_]+( [a-zA-Z]+)?) ([aAcCsS])([aAsS])?$', line.strip())
    if m == None : return False

    params['country'] = m.group(1).lower().title()
    params['cell1'] = m.group(3).lower()
    params['cell2'] = m.group(4).lower()
    return True

  def do_war_of_ideas(self, line):
    args = {}
    res = None

    if self.parse_target_country(line, args) == False :
      print("   Unknown usage - war_of_ideas %s\n" % line)
      self.help_war_of_ideas()
      return False

    res = self.game.war_of_ideas(args['country'])
  ...
  def complete_war_of_ideas(self, text, line, begidx, endidx) :
    if re.match("war_of_ideas $", line) != None :
      countries = self.game.board.get_allied_countries() + self.game.board.get_neutral_countries() + self.game.board.get_non_muslim_countries()

      return [ c.name for c in countries ]
    elif re.match('war_of_ideas ([a-zA-Z/_]+)$', line) != None :
      countries = self.game.board.get_allied_countries() + self.game.board.get_neutral_countries() + self.game.board.get_non_muslim_countries()
      if len(countries) == 1 : return [ countries[0].name ]
      return [ c.name for c in countries if c.name.lower().startswith(text.lower()) ]

    else : return []

  def help_war_of_ideas(self):
    print("   US Action - 7.2 War of Ideas")
    print("   Usage: war_of_ideas <country>")

  def do_disrupt(self, line):
    args = {}
    res = None

    if self.parse_disrupt(line, args) == False :
      print("   Unknown usage - disrupt %s\n" % line)
      self.help_disrupt()
      return False

    country = self.game.board.country(args['country'])
    if args['cell1'] == 'c' :
      res = self.game.disrupt(args['country'])
    elif args['cell1'] == 'a' and args['cell2'] == '' :
      res = self.game.disrupt(args['country'], 1)
    elif args['cell1'] == 'a' and args['cell2'] == 'a' :
      res = self.game.disrupt(args['country'], 2)
    elif args['cell1'] == 's' and args['cell2'] == '' :
      res = self.game.disrupt(args['country'], 0, 1)
    elif args['cell1'] == 's' and args['cell2'] == 's' :
      res = self.game.disrupt(args['country'], 0, 2)
    else :
      print("   Unknown usage - disrupt %s\n" % line)
      self.help_disrupt()
      return False
    ...
    if res[0] == False :
      if res[2] == 'no_active_cells' :
        print("   No active cells to disrupt")
      elif res[2] == 'no_sleeper_cells' :
        print("   No sleeper cells to disrupt")
      elif res[2] == 'ineligible_disrupt' :
        print("   Preconditions for disrupt are not met (7.4.1)")
      elif res[2] == 'no_jihad_presence' :
        print("   No cells or cadre present (7.4)")
      elif res[2] == 'no_cadre' :
  ...
  def complete_disrupt(self, text, line, begidx, endidx) :
    if re.match("disrupt $", line) != None :
      return [ c.name for c in self.game.board.get_countries_with_cells() ]
    elif re.match("disrupt ([a-zA-Z/_]+)$", line) != None:
      return [ c.name for c in self.game.board.get_countries_with_cells() if c.name.lower().startswith(text.lower()) ]
    elif re.match("disrupt ([a-zA-Z/_]+( [a-zA-Z]+)?) $", line) != None:
      c_name = re.match("disrupt ([a-zA-Z/_]+( [a-zA-Z]+)?) $", line).group(1)
      country = self.game.board.country(c_name)
      if country.cadre_Q() : return "C"
      if country.active_cells == 0 and country.sleeper_cells > 0 : return "S"
  ...
  def do_alert(self, line):
    ...
    res = self.game.alert(args['country'])
    if res[0] == False and res[2] == 'no_plots' :
      print("   %s does not have plots (7.5)" % args['country'])
    elif res[0] == True :
      print("   Remove single plot. %s now has %d plots" % (args['country'], res[2]))
    return False

  def complete_alert(self, text, line, begidx, endidx) :
    if re.match("alert $", line) != None and len(text) <= 0 :
      countries = self.game.board.get_countries_with_plots()
      if len(countries) == 1 : return [ countries[0].name ]
      return [ "%s (%d)" % (c.name, c.plots) for c in countries ]

    elif re.match('alert ([a-zA-Z/_]+)$', line) != None :
      countries = self.game.board.get_countries_with_plots()
      if len(countries) == 1 : return [ countries[0].name ]
      return [ c.name for c in countries if c.name.lower().startswith(text.lower()) ]
  ...
  def help_alert(self):
    print("   US Action - 7.5 Alert")
    print("   Usage: alert <country>")

  def do_reassessment(self, line) :
    res = self.reassessment()
    if res[0] == True : print("  US posture now %s" % self.game.board.country("United States").posture.value)
    elif res[0] == False : print("   US posture unchanged")

  def help_reassessment(self) :
    print("   US Action - 7.6 Reassessment")
    print("   Usage: reassessment")

  def do_withdraw(self, line) :
    args = {}

    if not self.parse_deploy(line, args) :
      print("   Unknown usage - withdraw %s\n" % line)
      self.help_withdraw()
      return False
    ...
    if res[0] == False :
      if res[4] == 'us_posture_hard' :
        print("    7.3.5 - No withdrawl with US Posture Hard\n")
      elif res[4] == 'not_regime_change' :
        print("    7.3.5 - %s is not a Regime Change country\n" % args['src'])
      elif res[4] == 'not_enough' :
        print("   Not enough troops in %s (%d) to withdraw" % (src, self.game.board.country(src).troops_stationed)) 
      elif res[4] == 'not_allied':
  ...
  def parse_deploy(self, line, params) :
    ...
    params['dst'] = m.group(1).lower().title()
    params['num'] = int(m.group(3))
    params['src'] = m.group(4).lower().title()
  ...
  def complete_deploy(self, text, line, begidx, endidx) :
    ...
    elif re.match('deploy ([a-zA-Z/_]+)$', line) != None:
      allied = self.game.board.get_allied_countries() + [self.game.board.troop_track]
      if len(allied) == 1 : return [ allied[0].name ]
      return [ c.name for c in allied if c.name.lower().startswith(text.lower()) ]

    elif re.match('deploy ([a-zA-Z/_]+( [a-zA-Z]+)?) $', line) != None:
      tc = self.game.board.get_troops_countries()
      if self.game.board.troop_track.troops() > 0 : tc += [self.game.board.troop_track]
      if len(tc) == 1 : return [ tc[0].troops() ]
      return ([ "%s (%d)" % (c.name, c.troops()) for c in tc ])

    elif re.match('deploy ([a-zA-Z/_]+( [a-zA-Z]+)?) ([0-9]+) $', line) != None:
      m = re.match('deploy ([a-zA-Z/_]+( [a-zA-Z]+)?) ([0-9]+) $', line)
      t = int(m.group(3))
    ...
    elif re.match('deploy ([a-zA-Z/_]+( [a-zA-Z]+)?) ([0-9]+) ([a-zA-Z/_]+)$', line) != None:
      m = re.match('deploy ([a-zA-Z/_]+( [a-zA-Z]+)?) ([0-9]+) ([a-zA-Z/_]+)$', line)
      t = int(m.group(3))
      tc = [ c for c in self.game.board.get_troops_countries() if c.troops_stationed >= t ]
      if self.game.board.troop_track.troops() >= t : tc += [self.game.board.troop_track]
      return [ c.name for c in tc if c.name.lower().startswith(text.lower()) ]
  ...
  def do_regime_change(self, line):
    ...
    if not self.parse_deploy(line, args) :
      print("   Unknown usage - regime_change %s\n" % line)
      self.help_regime_change()
      return False
    ...
    res = self.game.regime_change(dst, num, src)

    if res[0] == False :
      if res[4] == 'us_posture_soft' :
        print("   US posture is Soft; cannot perform regime change (7.3.4)")
      elif res[4] == 'already_regime_change' :
        print("   %s is already undergoing regime change" % dst)
      elif res[4] == 'min_troops' :
        print("   %d troops minimum to perform regime change (7.3.4)" % num)
      elif res[4] == 'not_islamist_rule' :
        print("   %d troops minimum to perform regime change (7.3.4)" % TROOPS_TO_DO_REGIME_CHANGE)
      elif res[4] == 'not_enough' :
        print("   Not enough troops in %s (%d)" % (src, self.game.board.country(src).troops_stationed))
      elif res[2] == 'regime_change_troop_restriction' :
        print("   %d troops must stay in %s (7.3.4)" % (TROOPS_UPKEEP_REGIME_CHANGE, src))
    elif res[0] == True :
        print("   Deployed to %s, %d troops, from %s; regime change in %s" % (dst, num, src, dst))

    return False

  def complete_regime_change(self, text, line, begidx, endidx) :
    if re.match("regime_change $", line) != None and len(text) <= 0 :
      allied = self.game.board.get_allied_countries()
      if len(allied) == 1 : return [ allied[0].name ]
      return [ "%s (%d)" % (c.name, c.troops()) for c in allied ] + [ "troop_track (%d)" % self.game.board.troop_track.troops() ]

    elif re.match('regime_change ([a-zA-Z/_]+)$', line) != None:
      allied = self.game.board.get_allied_countries() + [self.game.board.troop_track]
      if len(allied) == 1 : return [ allied[0].name ]
      return [ c.name for c in allied if c.name.lower().startswith(text.lower()) ]

    elif re.match('regime_change ([a-zA-Z/_]+( [a-zA-Z]+)?) $', line) != None:
      tc = self.game.board.get_troops_countries()
      if len(tc) == 1 : return [ tc[0].troops() ]
      return ([ "%s (%d)" % (c.name, c.troops()) for c in tc if c.regime_change_Q()])

    elif re.match('regime_change ([a-zA-Z/_]+( [a-zA-Z]+)?) ([0-9]+) $', line) != None:
      m = re.match('regime_change ([a-zA-Z/_]+( [a-zA-Z]+)?) ([0-9]+) $', line)
      t = int(m.group(3))
      tc = [ c for c in self.game.board.get_troops_countries() if c.troops_stationed >= t and c.regime_change_Q()]
      if len(tc) == 1 and tc[0].troops() >= t : return [ tc[0].name ]
      return ([ "%s (%d)" % (c.name, c.troops()) for c in tc ])

    elif re.match('regime_change ([a-zA-Z/_]+( [a-zA-Z]+)?) ([0-9]+) ([a-zA-Z/_]+)$', line) != None:
      m = re.match('regime_change ([a-zA-Z/_]+( [a-zA-Z]+)?) ([0-9]+) ([a-zA-Z/_]+)$', line)
      t = int(m.group(3))
      tc = [ c for c in self.game.board.get_troops_countries() if c.troops_stationed >= t and c.regime_change_Q()]
      return [ c.name for c in tc if c.name.lower().startswith(text.lower()) ]
  ...
  def help_regime_change(self):
    print("   US Action: Regime Change - 7.3.4: Special Deploy")
    print("   Usage: regime_change <to country> <num of troops> <from country>\n")
